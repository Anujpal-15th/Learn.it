...
**Next step (superseded — see the full design-system rewrite below):** commit + push + live browser verification against https://learn-it-roan.vercel.app.

---

## Full UI redesign v2 — dashboard prototype (backend explicitly frozen) — 2026-07-19

**Status:** dashboard done, code complete + builds clean; pushing + live-verifying next. Every other page (topic detail, LeetCode bank, Algorithms, Backend Topics, Networking, Interview Q&A, auth pages) still uses the OLD hard-border/grid look — deliberately, this is a prototype-first pass per plan below.

**Why:** user said the backend/content work is done ("we no need to touch back end") and asked for a full visual redesign — the strict black/white grid "dossier" aesthetic was described as "irritating," giving a headache. Gave real reference links (Dribbble animated-website tag, motion.dev/Framer Motion docs, LottieFiles, Jitter, a font-inspiration directory) and asked for "professional," not "AI-made," with real motion.

**Decision made without asking (stated to user, not a formal question):** the references given lean toward marketing-site/landing-page motion design. This app is a dense tracker (30 topics, 500+ checkable items) — a full Awwwards-style animated-hero treatment would actively fight its actual job. Synthesized instead toward the restrained, confident aesthetic of modern professional dev tools (Linear/Vercel/Raycast-style): soft surfaces, one accent color, real elevation via shadows instead of hard borders, purposeful micro-interactions via the Motion library rather than illustrative Lottie animations. Communicated this reasoning to the user directly before building, rather than either blindly following the marketing-site references or silently picking a direction.

**Plan (stated to user):** redesign the dashboard first as a concrete example, deploy it, get reaction/direction before rolling the same system out to the other ~9 pages — full-app-blind-redo risk avoided.

**Research done before building:** fetched motion.dev's quickstart directly (confirmed `npm install motion`, React support via `motion/react`, hover/press gestures, spring physics) rather than assuming API shape from memory.

**Built — dependency hygiene (found while installing `motion`):** `npm audit` surfaced a critical Next.js advisory bundle (cache poisoning, various DoS/SSRF vectors) on the pinned `next@14.2.5`. Patched to `next@14.2.35` — same-minor safe patch bump, applied deliberately (edited package.json + reinstalled, not a blind `--force`). One remaining "high" advisory needs Next 16 (a real major-version migration) to fully clear; left alone since it's a deliberate, separate task requiring its own testing, not something to fold into a UI pass — the specific vectors (image-optimization DoS, middleware/i18n bypass, WebSocket-upgrade SSRF) don't apply to this app anyway, since it uses none of those features.

**Built — new design system in `app/globals.css`:**
- New color tokens: `--ink` (soft #17181c, not pure black), `--paper`/`--paper2` (soft off-white / white), `--faint` (low-contrast borders, no longer a repeating grid), `--accent` (#6d5ef0 violet) + `--accent-ink`/`--accent-soft` (the one considered accent color — new, wasn't in the old strict-monochrome system), `--radius-sm/md/lg` (new — the old system had zero border-radius anywhere), `--shadow-sm/md/lg` (new — real elevation, replacing hard 1.5px borders). Same token set inverted for dark mode (deeper neutrals, lightened accent for contrast).
- Removed the persistent 48px checkerboard background grid from `body` entirely — almost certainly the actual source of the "headache" complaint (a fixed, full-viewport line pattern behind every page).
- `app/layout.js`: swapped fonts from Archivo/Archivo Black to **Sora** (display) + **Inter** (body) — the current-generation professional-product pairing (used by Linear, Vercel-adjacent tools). JetBrains Mono kept, but dialed back to genuinely tabular contexts only (was used almost everywhere before). Renamed the CSS font variables `--font-archivo`/`--font-archivo-black` → `--font-body`/`--font-display` throughout `globals.css` (45 occurrences, via `sed`) so the names describe their role, not a specific typeface that no longer loads.
- Redesigned dashboard-specific rules: `.boot`, `.topbar`/`.tab` (rounded pill buttons, accent-tinted hover instead of full-invert), `.hero`/`.eyebrow` (pill badge instead of a mono line-prefixed label), `.ledger` stat strip (4 individual rounded shadow cards instead of a hard-bordered spreadsheet strip), `.grid-days`/`.gday` (rounded cells, accent-toned fill levels instead of pure grayscale), `.phase-head`/`.phase-num` (pill badge), `.topic-grid`/`.card` (individual rounded shadow cards with gap between them, replacing the old shared-border "spreadsheet" grid and the black-panel-slides-up-on-hover interaction — hover is now a lift + shadow-grow + accent border), `.proj-card` (accent-tinted dashed-border card, replacing the diagonal hairline pattern fill), `.capstone-box` (rounded card with a soft accent gradient wash, replacing the hard 2px border + unicode-star flourish), `footer` (quiet, sentence case, no longer shouting uppercase mono).
- Left every non-dashboard page's specific classes (`.detail`, `.q-row`, `.sub-head`, `.checklist-columns`, `.concept-block`, `.back-btn`, `.auth-card`, etc.) untouched structurally — they now inherit the new base colors/fonts automatically (since they reference the same CSS variables) but still render with the old hard-border/sharp-corner treatment until redesigned in a follow-up pass.

**Built — real Motion usage in `app/page.js`** (not CSS-only animation): hero section now animates in via `motion.section` (`initial`/`animate`, replacing the old CSS `@keyframes rise`). Topic cards use a `motion.div` parent with `staggerChildren` + `whileInView` (each phase's card grid staggers in as it scrolls into view, once) and each card has spring-physics `whileHover`/`whileTap` (lift on hover, scale-down on tap) — removed the now-redundant CSS `transform` from `.card:hover` to avoid double-transform conflicts with Motion.

**Verified:** `npm run build` compiles clean. Bundle-size note (being upfront about it): the dashboard route's own JS grew from ~2.2kB to 42.5kB with Motion included (total First Load JS 175kB vs ~135kB before) — a real, known cost of adding a real animation library, not a regression to hide.

**Left to do:** push (no attribution) → Vercel redeploy → live verification: dashboard renders the new palette/typography/motion correctly in both light and dark, staggered card reveal and hover/tap physics work, grid background is gone. Then: show the user the live result and get their reaction/adjustments before rolling the same system out to the other ~9 pages (topic detail — including its two-column checklist layout and sidebar nav, LeetCode bank, Algorithms, Backend Topics, Networking, Interview Q&A, login/signup).

**Next step:** commit + push + live browser verification, then report the live URL back to the user for a design-direction check before continuing the rollout.
