...
import { topicSolved, topicComplete, quizComplete, projectDone, phaseProjectDone, qid, checklistProgress } from './topics.js';
...
// The first not-fully-done subtopic (checklist + practice both complete) —
// what the guided Topic page opens on, and what Home names as "current."
// Returns topic.subtopics.length once every subtopic is done (the caller
// then knows to point at the quiz/project step instead).
export function currentSubtopicIndex(topic, progress) {
  const idx = topic.subtopics.findIndex((s, si) => {
    const questDone = s.q.filter((_, qi) => progress && progress[qid(topic.id, si, qi)]).length;
    if (s.checklist) {
      const { c, t } = checklistProgress(topic, si, progress);
      return !(t > 0 && c === t && questDone === s.q.length);
    }
    return !(s.q.length > 0 && questDone === s.q.length);
  });
  return idx === -1 ? topic.subtopics.length : idx;
}
...
// The phase project tied to wherever the learner actually is on the core
// path (DSA/optional phases are skipped — they're not part of "what am I
// building right now"). Falls back to the last core phase once everything
// core is done, so there's always something to point at.
export function currentProject(roadmap, progress) {
  const stats = milestoneStats(roadmap, progress);
  const corePhaseIndices = stats
    .map((s) => s.index)
    .filter((i) => isCoreTrack(roadmap.phases[i].tier));
  if (corePhaseIndices.length === 0) return null;
  const currentIndex =
    corePhaseIndices.find((i) => !stats[i].done) ?? corePhaseIndices[corePhaseIndices.length - 1];
  const project = roadmap.phaseProjects[currentIndex];
  if (!project) return null;
  return {
    phaseIndex: currentIndex,
    phaseName: roadmap.phases[currentIndex].name,
    project,
    done: phaseProjectDone(roadmap.id, currentIndex, progress),
  };
}
