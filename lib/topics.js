...
export const PHASE_PROJECTS = [
  ...
  { title: 'Phase Project — Data Access Layer for a Bookstore', desc: 'A standalone JPA data layer over a normalized PostgreSQL schema with tuned queries and zero N+1 issues, plus a side-by-side MongoDB model of the same domain to compare relational vs document design.' },
  ...
  { title: 'Phase Project — Deployed, Event-Driven Service', desc: 'A containerized, cached, queue-driven service, deployed to Kubernetes (not just docker-compose) and auto-deployed via GitHub Actions.' },
  { title: 'Phase Project — System Design Portfolio', desc: 'Three written design docs (URL shortener, rate limiter, notification fan-out), with one of them actually deployed on real cloud infrastructure (managed DB, VPC, security groups) — the artifact interviewers actually want to see.' },
  { title: 'Phase Project — Full-Stack Roadmap App', desc: 'A React + TypeScript frontend wired to your Spring Boot API with auth — essentially this Ledger, rebuilt on your own stack, with the semantic HTML/CSS and vanilla JS fundamentals from earlier in this phase applied throughout, not skipped for the framework.' },
];
