...
export const PHASE_PROJECTS = [
  { title: 'Phase Project — CLI Inventory & Order Simulator', desc: 'A pure-Java console app (no Spring yet) applying OOP + SOLID + a UML-documented pattern set (Factory, Strategy, Observer), built with Gradle, version-controlled on GitHub, and started via your own bash script.' },
  { title: 'Phase Project — DSA Problem Tracker CLI', desc: 'A Java console app that logs solved problems across all 18 DSA patterns from this phase — arrays/two pointers/sliding window, stacks/binary search/linked lists, trees/heaps/tries, backtracking/graphs, advanced graphs/DP, and greedy/intervals/math/bit tricks — tracking your own stats by pattern and difficulty.' },
  { title: 'Phase Project — Data Access Layer for a Bookstore', desc: 'A normalized PostgreSQL schema (Databases & SQL) queried directly, then re-implemented as a JPA/Hibernate data layer (Hibernate/JPA) with tuned queries and zero N+1 issues, plus a side-by-side MongoDB model of the same domain (NoSQL Databases) to compare relational vs document design.' },
  { title: 'Phase Project — Task Management REST API', desc: 'A documented, versioned, paginated REST API (REST API Design) for managing projects/tasks/assignees, built on a Spring Boot IoC/DI container (Spring Core & Spring Boot) — proper DTOs, status codes, and error responses, the shape of a real placement-ready backend project.' },
  { title: 'Phase Project — Secured, Tested Booking API', desc: 'A booking/reservation API with JWT auth and RBAC (Security), backed by a full unit + integration test suite with mocking and coverage checks (Testing).' },
  { title: 'Phase Project — Deployed, Event-Driven Service', desc: 'A containerized service (Docker & Containers) with Redis caching and a message queue (Caching & Messaging), deployed to Kubernetes — not just docker-compose (Kubernetes & Orchestration) — with an automated GitHub Actions pipeline (CI/CD & Deployment).' },
  { title: 'Phase Project — System Design Portfolio', desc: 'Three written design docs applying System Design Basics (URL shortener, rate limiter, notification fan-out), with one actually deployed on real cloud infrastructure — managed DB, VPC, security groups (Cloud Fundamentals) — the artifact interviewers actually want to see.' },
  { title: 'Phase Project — Full-Stack Roadmap App', desc: 'A React + TypeScript frontend (React Fundamentals, React Advanced & State Management, TypeScript Fundamentals) wired to your Spring Boot API with auth — essentially this Ledger, rebuilt on your own stack, with the semantic HTML/CSS and vanilla JS/DOM fundamentals from earlier in this phase (HTML/CSS, JavaScript Core & DOM) applied throughout, not skipped for the framework.' },
];
...
export const TOPICS = [
  ...
  // ==================================================================
  {
    ...
    quiz: [
      { q: 'What does it mean that Java is statically typed?', options: ['A variable\'s type is fixed and checked by the compiler before the program runs', 'A variable\'s type can change at runtime', 'Types only matter for primitives, not objects', 'The type is inferred by the JVM at startup'], correct: 0, explanation: 'Static typing means the compiler checks and locks in a variable\'s type up front, unlike a dynamically typed language.' },
      { q: 'What is the default value of an uninitialized instance field of type boolean?', options: ['false', 'true', 'null', '0'], correct: 0, explanation: 'Java gives every uninitialized instance field a default — false for boolean, 0 for numeric primitives, null for references.' },
      { q: 'Which OOP pillar is demonstrated by keeping fields private and exposing behavior through methods?', options: ['Encapsulation', 'Inheritance', 'Polymorphism', 'Abstraction'], correct: 0, explanation: 'Encapsulation means an object controls its own state instead of letting outside code mutate fields directly.' },
      { q: 'Method overloading is resolved at which point?', options: ['Compile time, based on argument types', 'Runtime, based on the actual object type', 'Only for static methods', 'It is never resolved automatically'], correct: 0, explanation: 'Overloading is static (compile-time) dispatch; overriding is dynamic (runtime) dispatch — this distinction is a very common interview question.' },
      { q: 'What is the main advantage of a record over a hand-written immutable class?', options: ['One line replaces a constructor + getters + equals/hashCode/toString', 'Records are mutable by default', 'Records can extend other classes', 'Records replace interfaces entirely'], correct: 0, explanation: 'A record is a compact, immutable data carrier — ideal for DTOs — generating the boilerplate automatically.' },
    ],
  },
  {
    ...
    subtopics: [
      { title: 'Collections Framework',
        ...
        q: [
          ...
          { t: 'Implement a class used as a HashMap key, override equals()/hashCode() correctly, and write a test proving lookup fails if you only override one of them', d: 'M', p: 'Build task' } ] },
      ...
      { title: 'Modern Java — Virtual Threads (Project Loom)',
        ...
        learnMore: { label: 'Oracle — Virtual Threads', url: 'https://docs.oracle.com/en/java/javase/21/core/virtual-threads.html' },
        q: [
          { t: 'Rewrite a thread-pool-based I/O task runner using Executors.newVirtualThreadPerTaskExecutor()', d: 'M', p: 'Build task' },
          { t: 'Compare throughput of 10,000 blocking I/O tasks on platform threads vs virtual threads', d: 'M', p: 'Build task' },
          { t: 'Explain why virtual threads do NOT speed up a CPU-bound (non-blocking) computation', d: 'E', p: 'Build task' },
        ] },
    ],
    mini: { title: 'Checkpoint Project — Console Library Manager', desc: 'CLI app using collections, streams and custom exceptions to manage books, members and due dates in memory — with a virtual-thread-backed executor for simulated concurrent checkouts.' },
    quiz: [
      { q: 'Which collection gives O(1) average-case lookup by key?', options: ['HashMap', 'TreeMap', 'LinkedList', 'ArrayList'], correct: 0, explanation: 'HashMap uses hashing for near-constant-time get/put on average; TreeMap trades that for O(log n) sorted order.' },
      { q: 'What must a resource implement to be used in a try-with-resources block?', options: ['AutoCloseable', 'Serializable', 'Comparable', 'Cloneable'], correct: 0, explanation: 'try-with-resources calls close() automatically on anything implementing AutoCloseable, even if an exception is thrown.' },
      { q: 'A Java Stream is best described as:', options: ['Lazy and single-use — nothing runs until a terminal operation', 'Eager and reusable across multiple pipelines', 'A mutable in-memory collection', 'Always executed in parallel'], correct: 0, explanation: 'Nothing in a stream pipeline runs until a terminal operation like collect() or forEach() is called, and it can\'t be reused afterward.' },
      { q: 'What must you override alongside equals() to keep HashMap/HashSet working correctly?', options: ['hashCode()', 'toString()', 'compareTo()', 'clone()'], correct: 0, explanation: 'Equal objects must produce equal hash codes, or hash-based collections will silently fail to find entries you know are there.' },
      { q: 'What actually happens inside a HashMap when two different keys produce the same hashCode() (a collision)?', options: ['Both entries are chained (or tree-stored) in the same bucket, and equals() distinguishes them on lookup', 'The second insert silently overwrites the first, regardless of equals()', 'The HashMap throws an exception', 'The HashMap automatically resizes to avoid ever storing two entries in one bucket'], correct: 0, explanation: 'Collisions are normal and expected — Java handles them by storing multiple entries per bucket and using equals() to find the right one, which is also why a poor hashCode() (e.g. a constant) degrades a HashMap toward O(n).' },
      { q: 'What does the volatile keyword guarantee?', options: ['Visibility of a field\'s latest value across threads', 'Atomicity of compound operations like i++', 'That the field is immutable', 'That reads always hit a lock'], correct: 0, explanation: 'volatile only guarantees visibility, not atomicity — a classic interview trap question.' },
    ],
  },
  {
    id: 'solid', num: 3, phase: 0, title: 'SOLID & Design Principles',
    sub: 'The 5 SOLID principles, DRY/KISS/YAGNI, UML for communicating a design, and the full Gang-of-Four pattern catalog — creational, structural, and behavioral.',
    learnMore: { label: 'GeeksforGeeks — SOLID Principles', url: 'https://www.geeksforgeeks.org/system-design/solid-principle-in-programming-understand-with-real-life-examples/' },
    subtopics: [
      ...
      { title: 'DRY, KISS & YAGNI',
        ...
        learnMore: { label: 'GeeksforGeeks — Software Design Principles', url: 'https://www.geeksforgeeks.org/software-engineering/an-introduction-to-software-development-design-principles/' },
        checklist: [
        ...
        q: [
          { t: 'Find and eliminate 3 instances of duplicated business logic in a given codebase (DRY)', d: 'E', p: 'Build task' },
          { t: 'Simplify an over-engineered class using 3 layers of indirection for something a single method could do (KISS)', d: 'M', p: 'Build task' },
          { t: "Identify and remove a speculative 'plugin system' built for a requirement that never materialized (YAGNI)", d: 'M', p: 'Build task' } ] },
      { title: 'UML Basics',
        concepts: [
        ...
        learnMore: { label: 'GeeksforGeeks — UML Diagrams', url: 'https://www.geeksforgeeks.org/system-design/unified-modeling-language-uml-introduction/' },
        checklist: [
          'Class Diagrams',
          'Sequence Diagrams',
        ...
        q: [
          { t: 'Draw a class diagram for a Library system (Book, Member, Loan) showing multiplicities', d: 'E', p: 'Build task' },
          { t: 'Draw a sequence diagram for a checkout flow involving Cart, PaymentService, and OrderService', d: 'M', p: 'Build task' },
          { t: 'Explain, with an example, the difference between composition and aggregation in a UML diagram', d: 'E', p: 'Build task' } ] },
      { title: 'Creational Design Patterns',
        ...
        learnMore: { label: 'GeeksforGeeks — Creational Design Patterns', url: 'https://www.geeksforgeeks.org/system-design/creational-design-pattern/' },
        checklist: [
          'Singleton',
          'Factory Method',
          'Abstract Factory',
        ...
        q: [
          ...
          { t: 'Design an Abstract Factory that produces matching UI component families (dark/light theme buttons + dialogs)', d: 'M', p: 'Build task' },
          { t: 'Implement a Builder pattern for constructing a complex Pizza object with optional toppings', d: 'M', p: 'Build task' },
          { t: 'Implement Prototype to clone a complex Document object (with nested sections) instead of rebuilding it', d: 'M', p: 'Build task' } ] },
      { title: 'Structural Design Patterns',
        ...
        learnMore: { label: 'GeeksforGeeks — Structural Design Patterns', url: 'https://www.geeksforgeeks.org/system-design/structural-design-patterns/' },
        checklist: [
          'Adapter',
          'Facade',
          'Decorator',
        ...
        q: [
          ...
          { t: 'Build a Proxy that lazy-loads an expensive Image object only when it\'s actually displayed', d: 'M', p: 'Build task' },
          { t: 'Model a file system (File and Folder) using Composite so both can be treated uniformly', d: 'M', p: 'Build task' } ] },
      { title: 'Behavioral Design Patterns',
        ...
        learnMore: { label: 'GeeksforGeeks — Behavioral Design Patterns', url: 'https://www.geeksforgeeks.org/system-design/behavioral-design-patterns/' },
        checklist: [
          'Strategy',
          'Observer',
          'State',
        ...
        q: [
          ...
          { t: 'Model an order status (or traffic light) using the State pattern instead of a big switch statement', d: 'M', p: 'Build task' },
          { t: "Implement Command to support undo/redo on a text editor's operations", d: 'H', p: 'Build task' },
          { t: 'Use Template Method to define a common data-import skeleton (open, parse, validate, save) with format-specific subclasses', d: 'M', p: 'Build task' },
          { t: 'Implement Chain of Responsibility for a support-ticket escalation system (L1 → L2 → Manager)', d: 'M', p: 'Build task' } ] },
    ],
    mini: { title: 'Checkpoint Project — Pluggable Notification Engine', desc: 'A notification system (Email/SMS/Push) built against SOLID using a Factory + Strategy + Observer combination: add a new channel without touching existing classes, documented with a UML class diagram.' },
    quiz: [
      { q: 'What does the Single Responsibility Principle say?', options: ['A class should have one reason to change', 'A class should implement one interface', 'A class should have exactly one public method', 'A class should never be subclassed'], correct: 0, explanation: 'SRP is about reasons to change, not literal method/field counts — a class doing two unrelated jobs should be split.' },
      { q: 'What does the Open/Closed Principle recommend?', options: ['Open for extension, closed for modification', 'Every class must be open (non-final)', 'Classes should never be closed off with private fields', 'Modify existing code instead of adding new classes'], correct: 0, explanation: 'You should be able to add new behavior (e.g. a new Strategy) without editing existing, already-tested code.' },
      { q: 'Which principle does violating the Liskov Substitution Principle look like?', options: ['A subclass that breaks behavior callers expect from the base class', 'A subclass with more public methods than its parent', 'A class implementing too many interfaces', 'A method with too many parameters'], correct: 0, explanation: 'LSP means a subtype must be substitutable for its base type without breaking correctness — e.g. a Square extending Rectangle that breaks setWidth/setHeight independence.' },
      { q: 'What problem does Dependency Injection primarily solve?', options: ['Tight coupling that makes code hard to test and swap implementations', 'Slow compile times', 'Memory leaks', 'Thread synchronization'], correct: 0, explanation: 'DI lets a class depend on an abstraction supplied from outside, so tests can inject a fake and production can inject the real thing.' },
      { q: 'When would you generally prefer composition over inheritance?', options: ['When you want to reuse behavior without being locked into an "is-a" relationship', 'Whenever performance matters', 'Only in functional languages', 'Never — inheritance is always the safer default'], correct: 0, explanation: 'Composition ("has-a") is more flexible and avoids fragile deep inheritance hierarchies that couple classes tightly.' },
    ],
  },
  {
    ...
    quiz: [
      { q: 'What is the primary job of a build tool like Maven or Gradle?', options: ['Manage dependencies and automate the build/test/package lifecycle', 'Run the application in production', 'Replace version control', 'Compile-time type checking'], correct: 0, explanation: 'Build tools resolve dependencies, compile, run tests, and package your code into a deployable artifact, reproducibly.' },
      { q: 'In Maven, where are a project\'s dependencies declared?', options: ['pom.xml', 'build.gradle', 'settings.json', 'MANIFEST.MF'], correct: 0, explanation: 'pom.xml is Maven\'s project object model file, declaring dependencies, plugins, and build configuration.' },
      { q: 'What does "dependency scope" (e.g. test vs compile) control?', options: ['When and where a dependency is available (build, test, runtime)', 'The version number of the dependency', 'Whether the dependency is open source', 'The dependency\'s file size'], correct: 0, explanation: 'A test-scoped dependency (like JUnit) is available for tests but excluded from the final production artifact.' },
      { q: 'What is a key advantage Gradle is often cited for over Maven?', options: ['A more flexible, script-based build with incremental build caching', 'It requires no configuration at all', 'It only supports Java, not Kotlin', 'It cannot resolve transitive dependencies'], correct: 0, explanation: 'Gradle\'s Groovy/Kotlin DSL and build caching often make builds faster and more customizable than Maven\'s XML-based model.' },
      { q: 'What is a "transitive dependency"?', options: ['A dependency of your dependency, pulled in automatically', 'A dependency only used in tests', 'A circular dependency between two modules', 'A dependency resolved at runtime only'], correct: 0, explanation: 'If your dependency itself depends on other libraries, those get pulled into your project transitively unless excluded.' },
    ],
  },
  {
    ...
    quiz: [
      { q: 'What does `git rebase` do differently from `git merge`?', options: ['Rewrites commit history onto a new base instead of creating a merge commit', 'Deletes the branch being rebased', 'Only works on the main branch', 'Merges automatically without conflicts ever occurring'], correct: 0, explanation: 'Rebase replays your commits on top of another branch, producing a linear history, instead of merge\'s explicit merge commit.' },
      { q: 'What is a merge conflict caused by?', options: ['Git being unable to automatically reconcile changes to the same lines', 'A missing .gitignore file', 'Pushing to the wrong remote', 'Using tabs instead of spaces'], correct: 0, explanation: 'When two branches change the same lines differently, Git can\'t decide which change wins and asks you to resolve it manually.' },
      { q: 'What is the purpose of a .gitignore file?', options: ['Tell Git which files/patterns to never track', 'Encrypt sensitive files', 'List required dependencies', 'Configure remote URLs'], correct: 0, explanation: '.gitignore keeps build artifacts, secrets, and local config out of version control entirely.' },
      { q: 'What does `git cherry-pick` do?', options: ['Applies a specific commit from another branch onto your current branch', 'Deletes a specific commit from history', 'Picks a random commit to revert', 'Merges two unrelated repositories'], correct: 0, explanation: 'Cherry-pick lets you take one useful commit without merging the entire branch it came from.' },
      { q: 'In trunk-based development, how long do feature branches typically live?', options: ['Very short — merged back to main frequently', 'Months, until the feature is fully polished', 'Forever, as permanent parallel branches', 'They are never created'], correct: 0, explanation: 'Trunk-based development favors small, frequent merges to main over long-lived feature branches, reducing merge pain.' },
    ],
  },
  {
    id: 'linux-cli', num: 6, phase: 0, title: 'Linux & Command Line',
    sub: "The environment almost every backend service actually runs in — filesystem navigation, process management, and shell scripting. Skipped by most tutorials, assumed by every real job.",
    learnMore: { label: 'The Linux Command Line (free book)', url: 'https://linuxcommand.org/tlcl.php' },
    subtopics: [
      { title: 'Filesystem & Permissions',
        ...
        learnMore: { label: 'Linux Journey — Filesystem', url: 'https://linuxjourney.com/lesson/filesystem-hierarchy' },
        checklist: [
        ...
        q: [
          { t: 'Use find and grep to locate every .log file modified in the last 24 hours containing the word ERROR', d: 'E', p: 'Build task' },
          { t: "Fix a script that fails with 'permission denied' by diagnosing and correcting the file's permission bits", d: 'E', p: 'Build task' },
          { t: 'Explain the difference between a hard link and a symbolic link with a concrete example of each breaking', d: 'M', p: 'Build task' },
        ] },
      { title: 'Process Management & Networking',
        ...
        q: [
          { t: 'Start a long-running process, background it, then find and kill it by PID', d: 'E', p: 'Build task' },
          { t: 'Write a systemd unit file that keeps a Java app running and restarts it on crash', d: 'M', p: 'Build task' },
          { t: 'Use ss or netstat to find which process is holding port 8080 open', d: 'E', p: 'Build task' },
        ] },
      { title: 'Shell Scripting',
        ...
        learnMore: { label: 'Bash Reference Manual', url: 'https://www.gnu.org/software/bash/manual/bash.html' },
        checklist: [
        ...
        q: [
          { t: 'Write a bash script that backs up a directory to a timestamped archive and logs success/failure', d: 'M', p: 'Build task' },
          { t: 'Write a deploy script that checks the exit code of each step and aborts with a clear message on failure', d: 'M', p: 'Build task' },
          { t: 'Chain three commands with pipes to extract and count unique IP addresses from a log file', d: 'M', p: 'Build task' },
        ] },
    ],
    mini: { title: 'Checkpoint Project — Deployment Script', desc: 'A bash script that pulls the latest code, runs the test suite, and restarts a running service — checking the exit code after every step and logging what happened.' },
    quiz: [
      { q: 'What does the command `chmod +x script.sh` do?', options: ['Makes the script file executable', 'Compiles the script', 'Changes the file\'s owner', 'Deletes the file'], correct: 0, explanation: 'chmod changes file permissions; +x adds the executable bit so the script can be run directly.' },
      { q: 'What does a non-zero exit code from a command typically indicate?', options: ['The command failed or encountered an error', 'The command ran successfully', 'The command is still running', 'The command was skipped'], correct: 0, explanation: 'By convention, exit code 0 means success and any non-zero code signals some kind of failure a script should check for.' },
      { q: 'What is the purpose of piping (`|`) in the shell?', options: ['Send one command\'s output as the next command\'s input', 'Run two commands in parallel', 'Redirect output to a file', 'Comment out a line'], correct: 0, explanation: 'A pipe chains commands together, letting you compose small tools instead of writing one large program.' },
      { q: 'What does `grep -r "TODO" .` do?', options: ['Recursively searches all files under the current directory for "TODO"', 'Deletes all files containing TODO', 'Replaces TODO with an empty string', 'Lists only directories'], correct: 0, explanation: 'grep -r searches recursively; without it, grep only checks files in the current directory.' },
      { q: 'Why check a command\'s exit code in a deployment script rather than assuming success?', options: ['So the script can stop and alert instead of continuing after a silent failure', 'It has no real purpose in practice', 'Exit codes are only relevant for compiled languages', 'To make the script run faster'], correct: 0, explanation: 'A script that ignores failures can happily "deploy" a broken build — checking exit codes is how you catch that.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'dsa1', num: 7, phase: 1, title: 'DSA I — Arrays, Two Pointers & Sliding Window',
    ...
    subtopics: [
      { title: 'Arrays & Hashing',
        ...
        learnMore: { label: 'GeeksforGeeks — Hashing', url: 'https://www.geeksforgeeks.org/dsa/hashing-data-structure/' },
        q: [
      ...
      { title: 'Two Pointers',
        ...
        learnMore: { label: 'GeeksforGeeks — Two Pointer Technique', url: 'https://www.geeksforgeeks.org/dsa/two-pointers-technique/' },
        q: [
      ...
      { title: 'Sliding Window',
        ...
        learnMore: { label: 'GeeksforGeeks — Sliding Window Technique', url: 'https://www.geeksforgeeks.org/dsa/window-sliding-technique/' },
        q: [
    ...
    quiz: [
      { q: 'The two-pointer technique is most useful for:', options: ['Sorted arrays or problems where you scan from both ends inward', 'Unsorted graphs', 'Recursive tree traversal', 'Hash collisions'], correct: 0, explanation: 'Two pointers exploit sorted order (or a known invariant) to avoid an O(n²) nested loop.' },
      { q: 'What is the time complexity of the sliding window technique for finding a max-sum subarray of fixed size k?', options: ['O(n)', 'O(n²)', 'O(n log n)', 'O(2^n)'], correct: 0, explanation: 'A sliding window updates the sum incrementally as it slides, visiting each element a constant number of times.' },
      { q: 'What does a HashMap-based frequency counter give you that a nested loop doesn\'t?', options: ['O(1) average lookup/update instead of O(n) per check', 'Guaranteed sorted output', 'Thread safety', 'Lower memory usage'], correct: 0, explanation: 'Hashing turns "have I seen this before?" into a constant-time check instead of scanning what you\'ve seen so far.' },
      { q: 'Why use two pointers instead of a HashSet to detect a palindrome?', options: ['O(1) extra space instead of O(n)', 'It\'s always faster in Big-O time complexity', 'HashSets can\'t store characters', 'It handles Unicode better'], correct: 0, explanation: 'Two pointers comparing from both ends need no extra storage, while a HashSet-based approach would use O(n) space.' },
      { q: 'What is the main risk of an off-by-one error in a sliding window\'s window-size calculation?', options: ['Including one too many or too few elements, giving a wrong result', 'Causing a stack overflow', 'Making the algorithm run forever', 'Triggering a compiler error'], correct: 0, explanation: 'Sliding window bugs are almost always boundary/index mistakes — the algorithm compiles and runs, just returns the wrong answer.' },
    ],
  },
  {
    id: 'dsa2', num: 8, phase: 1, title: 'DSA II — Stack, Binary Search & Linked List',
    ...
    subtopics: [
      { title: 'Stack',
        ...
        learnMore: { label: 'GeeksforGeeks — Stack Data Structure', url: 'https://www.geeksforgeeks.org/dsa/stack-data-structure/' },
        q: [
      ...
      { title: 'Binary Search',
        ...
        learnMore: { label: 'GeeksforGeeks — Binary Search', url: 'https://www.geeksforgeeks.org/dsa/binary-search/' },
        q: [
      ...
      { title: 'Linked List',
        ...
        learnMore: { label: 'GeeksforGeeks — Linked List', url: 'https://www.geeksforgeeks.org/dsa/linked-list-data-structure/' },
        q: [
    ...
    quiz: [
      { q: 'What is the time complexity of binary search on a sorted array of n elements?', options: ['O(log n)', 'O(n)', 'O(n log n)', 'O(1)'], correct: 0, explanation: 'Binary search halves the search space every step, giving logarithmic time.' },
      { q: 'A stack is the natural data structure for which of these?', options: ['Matching balanced parentheses / undo history', 'Finding the shortest path in a graph', 'Sorting a list of numbers', 'Looking up a value by key'], correct: 0, explanation: 'A stack\'s LIFO order matches problems like bracket matching, undo stacks, and expression evaluation.' },
      { q: 'How do you detect a cycle in a singly linked list in O(1) space?', options: ['Floyd\'s tortoise-and-hare (slow/fast pointer)', 'Store every node in a HashSet', 'Reverse the list twice', 'Count the nodes'], correct: 0, explanation: 'Two pointers moving at different speeds will eventually meet if and only if there\'s a cycle — no extra memory needed.' },
      { q: 'What is the worst-case time complexity of binary search on an UNSORTED array?', options: ['Binary search doesn\'t work correctly on unsorted data at all', 'O(log n), same as sorted', 'O(1)', 'O(n²)'], correct: 0, explanation: 'Binary search relies entirely on the sorted-order invariant — running it on unsorted data gives meaningless results, not just a slower runtime.' },
      { q: 'Reversing a singly linked list iteratively requires tracking which pointers?', options: ['previous, current, and next', 'Only the head', 'Only the tail', 'The list length'], correct: 0, explanation: 'You need all three to safely redirect each node\'s next pointer without losing the rest of the list.' },
    ],
  },
  {
    id: 'dsa3', num: 9, phase: 1, title: 'DSA III — Trees, Heaps & Tries',
    ...
    subtopics: [
      { title: 'Trees',
        ...
        learnMore: { label: 'GeeksforGeeks — Binary Tree', url: 'https://www.geeksforgeeks.org/dsa/binary-tree-data-structure/' },
        q: [
      ...
      { title: 'Heaps & Priority Queue',
        ...
        learnMore: { label: 'GeeksforGeeks — Heap Data Structure', url: 'https://www.geeksforgeeks.org/dsa/heap-data-structure/' },
        q: [
      ...
      { title: 'Tries',
        ...
        learnMore: { label: 'GeeksforGeeks — Trie', url: 'https://www.geeksforgeeks.org/dsa/trie-insert-and-search/' },
        q: [
    ...
    quiz: [
      { q: 'What makes a Trie efficient for prefix-based lookups like autocomplete?', options: ['Shared prefixes are stored once, so lookup is O(word length), not O(n words)', 'It sorts all words alphabetically first', 'It hashes every word', 'It only works for numbers'], correct: 0, explanation: 'A Trie\'s structure directly encodes shared prefixes, making prefix search proportional to the query length, not the dataset size.' },
      { q: 'In a valid Binary Search Tree, an in-order traversal visits nodes:', options: ['In sorted ascending order', 'In random order', 'Level by level', 'In reverse insertion order'], correct: 0, explanation: 'BST\'s left < node < right invariant means in-order traversal naturally produces sorted output.' },
      { q: 'What operation does a min-heap guarantee O(log n) for?', options: ['Insert and extract-min', 'Random access by index', 'Searching for an arbitrary value', 'Sorting in O(n)'], correct: 0, explanation: 'A heap keeps the minimum at the root, and insert/extract-min both cost O(log n) due to the sift-up/sift-down.' },
      { q: 'What is the worst-case time complexity of search in an unbalanced BST?', options: ['O(n), if the tree degenerates into a linked list', 'O(log n), always', 'O(1)', 'O(n²)'], correct: 0, explanation: 'Without balancing (like AVL/Red-Black trees), a BST built from sorted input can degenerate into a linked list.' },
      { q: 'Why is a heap the right structure for a "run the highest-priority job next" scheduler?', options: ['It gives O(log n) insert and O(1) peek at the highest priority item', 'It keeps items in insertion order', 'It requires no comparisons', 'It automatically deletes stale jobs'], correct: 0, explanation: 'A priority queue backed by a heap is the standard structure for always accessing the current max/min efficiently.' },
    ],
  },
  {
    id: 'dsa4', num: 10, phase: 1, title: 'DSA IV — Backtracking & Graphs',
    ...
    subtopics: [
      { title: 'Backtracking',
        ...
        learnMore: { label: 'GeeksforGeeks — Backtracking', url: 'https://www.geeksforgeeks.org/dsa/backtracking-algorithms/' },
        q: [
      ...
      { title: 'Graphs',
        ...
        learnMore: { label: 'GeeksforGeeks — Graph Data Structure', url: 'https://www.geeksforgeeks.org/dsa/graph-data-structure-and-algorithms/' },
        q: [
    ...
    quiz: [
      { q: 'What is the core idea behind backtracking?', options: ['Try a choice, recurse, and undo it if it doesn\'t lead to a solution', 'Always choose the locally best option and never revisit it', 'Solve subproblems once and cache them', 'Process items level by level'], correct: 0, explanation: 'Backtracking explores a decision tree and prunes/undoes ("backtracks") branches that fail, unlike greedy or DP approaches.' },
      { q: 'BFS explores a graph:', options: ['Layer by layer, using a queue', 'As deep as possible before backtracking, using a stack/recursion', 'In random order', 'Only along the shortest edge weight'], correct: 0, explanation: 'BFS visits all neighbors at the current distance before going further, which is why it finds shortest paths in unweighted graphs.' },
      { q: 'What data structure is DFS naturally implemented with (besides recursion)?', options: ['A stack', 'A queue', 'A heap', 'A hash map'], correct: 0, explanation: 'DFS explores as deep as possible before backtracking — a stack (or the call stack via recursion) captures that "go deep, then pop back" behavior.' },
      { q: 'Why does BFS guarantee the shortest path in an UNWEIGHTED graph, but DFS does not?', options: ['BFS explores nodes in increasing distance order from the source', 'BFS visits every node twice', 'DFS never terminates', 'BFS uses less memory'], correct: 0, explanation: 'Because BFS expands the frontier one layer at a time, the first time it reaches a node is guaranteed to be via a shortest path.' },
      { q: 'In a backtracking maze solver, what prevents infinite loops when the maze has cycles?', options: ['Marking visited cells before recursing further', 'Using a bigger stack size', 'Sorting the moves', 'Backtracking cannot handle cycles at all'], correct: 0, explanation: 'Without a visited-check, backtracking can revisit the same cell forever — marking visited cells (and unmarking on backtrack, if paths must be reusable) is essential.' },
    ],
  },
  {
    id: 'dsa5', num: 11, phase: 1, title: 'DSA V — Advanced Graphs & Dynamic Programming',
    sub: 'Weighted graphs and shortest-path algorithms, plus the two-dimensional thinking behind dynamic programming.',
    learnMore: { label: 'GeeksforGeeks — Dynamic Programming', url: 'https://www.geeksforgeeks.org/dsa/dynamic-programming/' },
    subtopics: [
      { title: 'Advanced Graphs',
        ...
        learnMore: { label: 'GeeksforGeeks — Advanced Graph Algorithms', url: 'https://www.geeksforgeeks.org/dsa/dijkstras-shortest-path-algorithm-greedy-algo-7/' },
        q: [
      ...
      { title: '2-D Dynamic Programming',
        ...
        learnMore: { label: 'GeeksforGeeks — 2D Dynamic Programming', url: 'https://www.geeksforgeeks.org/dsa/dp-on-grids/' },
        q: [
    ...
    quiz: [
      { q: 'Why doesn\'t Dijkstra\'s algorithm work correctly with negative edge weights?', options: ['It assumes once a node is finalized its shortest distance can\'t improve, which negative edges can violate', 'It doesn\'t support weighted graphs at all', 'It only works on trees', 'It requires the graph to be undirected'], correct: 0, explanation: 'Dijkstra greedily finalizes the closest unvisited node — a later negative edge could still find a shorter path, breaking that assumption.' },
      { q: 'What is the defining property of a problem solvable with dynamic programming?', options: ['Optimal substructure and overlapping subproblems', 'It must be solvable with a single greedy pass', 'It must involve a graph', 'It has no valid recursive solution'], correct: 0, explanation: 'DP caches solutions to overlapping subproblems that combine into an optimal solution — without both properties, DP doesn\'t help.' },
      { q: 'What does memoization add to a plain recursive solution?', options: ['Caching results of subproblems already solved, avoiding recomputation', 'Parallel execution', 'Automatic base-case detection', 'Guaranteed O(1) time'], correct: 0, explanation: 'Memoization turns an exponential naive recursion (e.g. Fibonacci) into polynomial time by reusing cached subproblem answers.' },
      { q: 'Bellman-Ford is preferred over Dijkstra specifically when:', options: ['The graph may have negative edge weights', 'The graph is very large', 'The graph is a tree', 'You need the fastest possible algorithm'], correct: 0, explanation: 'Bellman-Ford is slower than Dijkstra but correctly handles negative weights and can detect negative cycles.' },
      { q: 'In the classic knapsack DP, what do the two dimensions of the DP table typically represent?', options: ['Items considered so far, and remaining capacity', 'Row and column of the input matrix', 'Time and space complexity', 'Number of graph nodes and edges'], correct: 0, explanation: 'The state (item index, remaining capacity) captures everything needed to decide the optimal value from that point forward.' },
    ],
  },
  {
    id: 'dsa6', num: 12, phase: 1, title: 'DSA VI — Greedy, Intervals, Math & Bit Manipulation',
    ...
    learnMore: { label: 'GeeksforGeeks — Greedy Algorithms', url: 'https://www.geeksforgeeks.org/dsa/greedy-algorithms/' },
    subtopics: [
      { title: 'Greedy',
        concepts: [
      ...
      { title: 'Intervals',
        ...
        learnMore: { label: 'GeeksforGeeks — Interval Scheduling', url: 'https://www.geeksforgeeks.org/dsa/merging-intervals/' },
        q: [
      ...
      { title: 'Math & Geometry',
        ...
        learnMore: { label: 'GeeksforGeeks — Mathematical Algorithms', url: 'https://www.geeksforgeeks.org/dsa/mathematical-algorithms/' },
        q: [
      ...
      { title: 'Bit Manipulation',
        ...
        learnMore: { label: 'GeeksforGeeks — Bit Manipulation', url: 'https://www.geeksforgeeks.org/dsa/all-about-bit-manipulation/' },
        q: [
    ...
    quiz: [
      { q: 'A greedy algorithm makes decisions:', options: ['Locally optimal at each step, without reconsidering earlier choices', 'By trying every possible combination', 'Only after sorting the entire input twice', 'By backtracking whenever a step fails'], correct: 0, explanation: 'Greedy commits to the best-looking local choice and never revisits it — correct only when the problem has the greedy-choice property.' },
      { q: 'When merging overlapping intervals, what\'s the first step that makes the rest of the algorithm simple?', options: ['Sort the intervals by start time', 'Sort the intervals by length', 'Reverse the input', 'Convert intervals to a graph'], correct: 0, explanation: 'Once sorted by start, you only ever need to compare each interval against the last merged one, giving an O(n log n) solution.' },
      { q: 'The XOR bit trick a ^ a = 0 is commonly used to solve which type of problem?', options: ['Finding the single non-duplicated element in an array of pairs', 'Sorting an array', 'Finding the maximum subarray sum', 'Detecting a cycle in a linked list'], correct: 0, explanation: 'XORing every element cancels out pairs, leaving only the element that appears once — O(n) time, O(1) space.' },
      { q: 'Why can a greedy solution fail on a problem that actually needs dynamic programming?', options: ['A locally optimal choice can lock out a better global solution', 'Greedy algorithms are always slower', 'Greedy algorithms cannot use sorting', 'DP problems never have a greedy-looking structure'], correct: 0, explanation: 'Classic example: coin change with arbitrary denominations — the greedy largest-coin-first approach doesn\'t always give the minimum number of coins.' },
      { q: 'What is the time complexity of checking if two intervals overlap?', options: ['O(1)', 'O(n)', 'O(log n)', 'O(n²)'], correct: 0, explanation: 'Comparing two intervals\' start/end values is a fixed number of comparisons regardless of input size.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'sql-db', num: 13, phase: 2, title: 'Databases & SQL',
    ...
    subtopics: [
      ...
      { title: 'JDBC Fundamentals',
        ...
        learnMore: { label: 'Oracle — JDBC Basics', url: 'https://docs.oracle.com/javase/tutorial/jdbc/basics/index.html' },
        checklist: [
        ...
        q: [
          { t: 'Write a raw JDBC program that connects to PostgreSQL and runs a parameterized SELECT', d: 'E', p: 'Build task' },
          { t: 'Demonstrate a SQL injection via string-concatenated Statement, then fix it with PreparedStatement', d: 'M', p: 'Build task' },
          { t: 'Write a JDBC insert wrapped in try-with-resources that correctly closes the Connection and Statement', d: 'M', p: 'Build task' },
        ] },
    ],
    ...
    quiz: [
      { q: 'What is the difference between an INNER JOIN and a LEFT JOIN?', options: ['LEFT JOIN keeps all left-table rows even without a match; INNER JOIN only keeps matches', 'They always produce identical results', 'INNER JOIN is always faster', 'LEFT JOIN only works on indexed columns'], correct: 0, explanation: 'A LEFT JOIN preserves every row from the left table, filling unmatched right-table columns with NULL.' },
      { q: 'What does the ACID "I" (Isolation) guarantee?', options: ['Concurrent transactions don\'t see each other\'s uncommitted changes', 'Data is never lost after a crash', 'A transaction is all-or-nothing', 'Every row has a unique key'], correct: 0, explanation: 'Isolation controls what one transaction can see of another\'s in-progress changes, governed by the isolation level.' },
      { q: 'Why wouldn\'t you index every column in a table?', options: ['Each index adds write overhead and storage cost', 'Databases only allow one index per table', 'Indexes make SELECT queries slower', 'Indexes are deprecated in modern SQL'], correct: 0, explanation: 'Every index must be updated on every INSERT/UPDATE/DELETE — indexing everything trades write speed for read speed you may not need.' },
      { q: 'What normal form eliminates transitive dependencies on the primary key?', options: ['Third Normal Form (3NF)', 'First Normal Form (1NF)', 'Boyce-Codd Normal Form only', 'Fourth Normal Form (4NF)'], correct: 0, explanation: '3NF requires every non-key column to depend only on the key, not on another non-key column.' },
      { q: 'What is a database transaction primarily used to guarantee?', options: ['A group of operations either all succeed or all roll back together', 'Faster query execution', 'Automatic indexing', 'Data replication across servers'], correct: 0, explanation: 'Transactions give you atomicity — critical for operations like transferring money between two accounts.' },
    ],
  },
  {
    id: 'jpa', num: 14, phase: 2, title: 'Hibernate / JPA',
    ...
    quiz: [
      { q: 'What is the N+1 query problem?', options: ['Fetching a list, then issuing one extra query per item for related data', 'Running the same query N times by mistake', 'A migration that adds N+1 columns', 'A deadlock between N transactions'], correct: 0, explanation: 'Lazy-loaded associations accessed in a loop trigger one query per row, instead of one join query — a very common JPA performance bug.' },
      { q: 'What is the difference between FetchType.LAZY and FetchType.EAGER?', options: ['LAZY loads the association only when accessed; EAGER loads it immediately with the parent', 'LAZY is faster in every case', 'EAGER only works with @OneToMany', 'They behave identically in practice'], correct: 0, explanation: 'LAZY defers the extra query until the association is actually touched, which is usually safer against accidental N+1s.' },
      { q: 'What does @Transactional on a Spring service method do?', options: ['Wraps the method in a database transaction, committing or rolling back as a unit', 'Marks the method as thread-safe', 'Caches the method\'s return value', 'Makes the method asynchronous'], correct: 0, explanation: 'It\'s Spring\'s declarative way of applying transaction boundaries via an AOP proxy around the method.' },
      { q: 'How would you fix an N+1 problem for a @OneToMany association in JPA?', options: ['Use a JOIN FETCH (or an entity graph) to load it in one query', 'Switch the database to NoSQL', 'Disable the ORM cache', 'Add more indexes'], correct: 0, explanation: 'JOIN FETCH tells JPA to eagerly fetch the association in the same query, avoiding the per-row follow-up queries.' },
      { q: 'Why does calling an @Transactional method from within the same class often fail to start a transaction?', options: ['Spring\'s AOP proxy is bypassed on internal (self) method calls', 'Transactional annotations only work on interfaces', 'The method must be static', 'JPA disables transactions inside loops'], correct: 0, explanation: 'The proxy that applies @Transactional wraps external calls to the bean — a direct `this.method()` call skips the proxy entirely.' },
    ],
  },
  {
    id: 'nosql', num: 15, phase: 2, title: 'NoSQL Databases',
    ...
    learnMore: { label: 'MongoDB — Manual', url: 'https://www.mongodb.com/docs/manual/' },
    subtopics: [
      { title: 'Document & Key-Value Stores',
        ...
        learnMore: { label: 'Redis — Data Types', url: 'https://redis.io/docs/latest/develop/data-types/' },
        checklist: [
        ...
        q: [
          { t: 'Model a product catalog as MongoDB documents, choosing embedding vs referencing for variants and reviews', d: 'M', p: 'Build task' },
          { t: 'Build a leaderboard using a Redis sorted set instead of a SQL table with ORDER BY', d: 'M', p: 'Build task' },
          { t: 'Write and run 5 basic MongoDB CRUD queries (find, filter, update, aggregate)', d: 'E', p: 'Build task' },
        ] },
      { title: 'SQL vs NoSQL Trade-offs',
        ...
        q: [
          { t: 'Write a short design doc comparing a relational vs document-store schema for a social media feed', d: 'M', p: 'Build task' },
          { t: 'Explain a real scenario where eventual consistency would cause a visible, user-facing bug', d: 'M', p: 'Build task' },
          { t: 'Decide SQL vs NoSQL for three different given scenarios (banking, product catalog, chat messages) and justify each', d: 'M', p: 'Build task' },
        ] },
    ],
    mini: { title: 'Checkpoint Project — Dual-Store Activity Feed', desc: 'Store user activity events in MongoDB (or a similar document store) and compare the query patterns and schema design against an equivalent relational table — same data, both approaches, side by side.' },
    quiz: [
      { q: 'What is the core design difference between a document store (MongoDB) and a relational database?', options: ['Data is modeled around how it\'s read, not normalized across tables', 'Document stores don\'t support indexes', 'Relational databases can\'t store JSON', 'Document stores never scale horizontally'], correct: 0, explanation: 'A document store embeds related data together for read efficiency, trading off the normalization discipline of a relational schema.' },
      { q: 'What is database sharding?', options: ['Splitting one logical database across multiple physical machines by a key', 'Creating a backup copy of the database', 'Adding an index to speed up queries', 'Encrypting data at rest'], correct: 0, explanation: 'Sharding is how you scale writes/storage beyond what a single machine can hold, at the cost of cross-shard query complexity.' },
      { q: 'When would a document store genuinely be the better fit than a relational database?', options: ['When your data is naturally hierarchical/nested and rarely needs cross-entity joins', 'Whenever you need strong multi-row ACID transactions', 'Whenever your schema is highly relational', 'Always — document stores are a strict upgrade'], correct: 0, explanation: 'Document stores shine for read-heavy, embeddable data (e.g. a user profile with nested settings) — not for heavily joined, transactional data.' },
      { q: 'What is a key trade-off of denormalizing (embedding) data in a document store?', options: ['Updates to shared data may need to touch many documents', 'It always uses less storage than normalization', 'It makes joins faster than SQL', 'It removes the need for indexes'], correct: 0, explanation: 'Embedding avoids joins at read time but means the same fact can live in multiple documents, complicating updates.' },
      { q: 'What does database replication provide?', options: ['Read scaling and failover via copies of data on other machines', 'Automatic schema migration', 'Query result caching', 'Data compression'], correct: 0, explanation: 'A replica can serve read traffic and take over if the primary fails, though it doesn\'t by itself solve write scaling.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'spring', num: 16, phase: 3, title: 'Spring Core & Spring Boot',
    ...
    quiz: [
      { q: 'What is Inversion of Control (IoC)?', options: ['The framework, not your code, controls object creation and wiring', 'A design pattern for sorting algorithms', 'A way to reverse a collection', 'A database transaction isolation level'], correct: 0, explanation: 'Instead of your code calling `new` to build dependencies, the container creates and injects them — inverting who\'s in control.' },
      { q: 'What is the default Spring bean scope?', options: ['Singleton — one shared instance per container', 'Prototype — a new instance every injection', 'Request — one per HTTP request', 'Session — one per user session'], correct: 0, explanation: 'Singleton is the default; you opt into prototype/request/session scope explicitly when you need a different lifecycle.' },
      { q: 'What does Spring Boot autoconfiguration actually do?', options: ['Conditionally configures beans based on what\'s on the classpath and existing config', 'Manually writes your configuration files for you', 'Disables all configuration by default', 'Only works with XML config'], correct: 0, explanation: '@Conditional annotations under the hood decide whether to configure a bean, based on classpath presence and existing beans.' },
      { q: 'What does Spring AOP (Aspect-Oriented Programming) let you do?', options: ['Apply cross-cutting logic (logging, transactions) without modifying the target class', 'Replace object-oriented design entirely', 'Compile Java to bytecode faster', 'Automatically generate REST endpoints'], correct: 0, explanation: 'AOP proxies wrap a bean\'s methods to inject behavior like @Transactional or custom logging aspects, separate from business logic.' },
      { q: 'Why would you use @Qualifier alongside @Autowired?', options: ['To disambiguate which bean to inject when multiple beans implement the same interface', 'To mark a bean as a singleton', 'To skip dependency injection entirely', 'To enable component scanning'], correct: 0, explanation: 'When Spring finds more than one matching bean, @Qualifier tells it exactly which one you want.' },
    ],
  },
  {
    id: 'rest', num: 17, phase: 3, title: 'REST API Design',
    ...
    quiz: [
      { q: 'What does it mean for an HTTP method to be idempotent?', options: ['Calling it once or many times has the same effect on server state', 'It always returns the same response body', 'It can only be called once', 'It requires authentication'], correct: 0, explanation: 'GET, PUT, and DELETE are idempotent; POST typically isn\'t — this matters a lot for safe retries.' },
      { q: 'What is the difference between PUT and PATCH?', options: ['PUT replaces the whole resource; PATCH applies a partial update', 'PUT is read-only; PATCH writes data', 'PATCH requires no request body', 'They are functionally identical'], correct: 0, explanation: 'PUT sends the full new representation, while PATCH sends only the fields that changed.' },
      { q: 'What HTTP status code should a successful DELETE typically return?', options: ['204 No Content (or 200 with a body)', '404 Not Found', '500 Internal Server Error', '301 Moved Permanently'], correct: 0, explanation: '204 signals success with nothing further to return — a common, correct choice for DELETE.' },
      { q: 'Why is offset-based pagination (LIMIT/OFFSET) problematic at large scale?', options: ['The database still has to scan and discard all skipped rows', 'It cannot return results in order', 'It requires no index at all', 'It only works with NoSQL databases'], correct: 0, explanation: 'A large OFFSET forces the database to walk past many rows before returning results — cursor-based pagination avoids this.' },
      { q: 'What is a common, safe approach to versioning a public REST API?', options: ['A version segment in the URL path (e.g. /api/v1/...)', 'Never versioning and just changing behavior in place', 'Requiring clients to guess the schema', 'Only versioning the database, not the API'], correct: 0, explanation: 'URL-path versioning is explicit and simple for clients to pin to, though header-based versioning is a valid alternative.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'security', num: 18, phase: 4, title: 'Security',
    ...
    quiz: [
      { q: 'What is the difference between authentication and authorization?', options: ['Authentication proves who you are; authorization decides what you\'re allowed to do', 'They are the same concept with different names', 'Authorization always happens before authentication', 'Authentication only applies to APIs, not web apps'], correct: 0, explanation: 'You authenticate once (login), then every subsequent request is authorized against your roles/permissions.' },
      { q: 'Why should passwords be hashed with a slow algorithm like bcrypt instead of a fast one like SHA-256?', options: ['Slowness makes brute-force and rainbow-table attacks impractical', 'bcrypt encrypts instead of hashing', 'Fast hashes are always insecure to store', 'SHA-256 cannot be used for passwords at all'], correct: 0, explanation: 'bcrypt is deliberately slow and salted, making it computationally expensive to try billions of guesses.' },
      { q: 'What does JWT stand for and what does it typically carry?', options: ['JSON Web Token — a signed, self-contained set of claims about the user', 'Java Web Task — a scheduled background job', 'JSON Write Trigger — a database hook', 'Just Web Traffic — an HTTP log format'], correct: 0, explanation: 'A JWT is signed so the server can verify it wasn\'t tampered with, without a database lookup on every request.' },
      { q: 'What does CSRF (Cross-Site Request Forgery) exploit?', options: ['A browser automatically sending cookies with a request from another site', 'A SQL injection in a login form', 'A weak password hash', 'An unencrypted HTTPS connection'], correct: 0, explanation: 'CSRF tricks a logged-in user\'s browser into making an unwanted request, relying on cookies being sent automatically — CSRF tokens defend against this.' },
      { q: 'What is the primary defense against SQL injection?', options: ['Parameterized queries / prepared statements instead of string concatenation', 'Encrypting the entire database', 'Rate limiting requests', 'Using a NoSQL database instead'], correct: 0, explanation: 'Parameterized queries separate code from data, so user input can never be interpreted as SQL syntax.' },
    ],
  },
  {
    id: 'testing', num: 19, phase: 4, title: 'Testing',
    ...
    quiz: [
      { q: 'What is the main difference between a unit test and an integration test?', options: ['A unit test isolates one component (often with mocks); an integration test exercises real collaborators together', 'Integration tests never use assertions', 'Unit tests always require a database', 'They test the exact same thing, just at different speeds'], correct: 0, explanation: 'The test pyramid favors many fast, isolated unit tests and fewer, slower integration tests that catch wiring issues.' },
      { q: 'What is the difference between a mock and a stub?', options: ['A mock verifies interactions happened; a stub just returns canned responses', 'A stub is faster than a mock in every case', 'Mocks can only be used in Python', 'They are interchangeable terms with no real difference'], correct: 0, explanation: 'You assert against a mock ("was this method called?"); a stub just feeds fixed data to the code under test.' },
      { q: 'Why would you prefer Testcontainers over an in-memory H2 database for integration tests?', options: ['H2 can behave differently from your real production database (Postgres, MySQL)', 'Testcontainers is faster to start than H2', 'H2 cannot run any SQL at all', 'Testcontainers requires no Docker'], correct: 0, explanation: 'Testcontainers spins up the real database in a container, catching dialect-specific bugs H2 would silently hide.' },
      { q: 'What is the red-green-refactor cycle in TDD?', options: ['Write a failing test, make it pass minimally, then improve the code', 'Write all tests after the feature is complete', 'Refactor first, then write tests', 'Only refactor code that\'s currently broken'], correct: 0, explanation: 'TDD\'s cycle keeps you honest that the test actually fails first (red), then passes (green), then gets cleaned up.' },
      { q: 'Why isn\'t 100% code coverage the same as "well tested"?', options: ['Coverage measures lines executed, not whether the right assertions were made', 'Coverage tools are always inaccurate', '100% coverage is impossible to achieve', 'Coverage only applies to integration tests'], correct: 0, explanation: 'A test can execute every line and assert nothing meaningful — coverage is a floor, not proof of correctness.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'cache-mq', num: 20, phase: 5, title: 'Caching & Messaging',
    ...
    quiz: [
      { q: 'What problem does Redis caching primarily solve?', options: ['Skipping a slow database round-trip for frequently-read data', 'Guaranteeing data is never lost', 'Replacing the need for a database entirely', 'Encrypting data at rest'], correct: 0, explanation: 'Keeping hot data in memory avoids repeatedly hitting a slower disk-backed database for the same reads.' },
      { q: 'What is a cache stampede?', options: ['Many requests miss the cache at once and all hit the database simultaneously', 'The cache growing too large and crashing', 'A cache key colliding with another key', 'The database overwriting cached data'], correct: 0, explanation: 'A stampede happens when a popular cache entry expires and a flood of requests all race to recompute it at once — locking/coalescing prevents it.' },
      { q: 'Why must a message consumer be idempotent in an event-driven system?', options: ['Message brokers can deliver the same message more than once', 'Idempotency makes messages process faster', 'It\'s required by the JMS specification', 'Idempotency prevents any message loss'], correct: 0, explanation: 'At-least-once delivery is common — a consumer that isn\'t idempotent could double-charge or double-ship on a redelivered message.' },
      { q: 'What is the main trade-off of a write-through cache strategy?', options: ['Every write goes to both cache and database, adding write latency for read consistency', 'It never needs invalidation', 'It only works with NoSQL', 'It removes the need for a database'], correct: 0, explanation: 'Write-through keeps cache and DB in sync at write time, at the cost of slower writes compared to write-behind.' },
      { q: 'What does a message queue decouple in a system?', options: ['Producers from consumers, letting them operate at different speeds independently', 'The frontend from the CSS framework', 'Unit tests from integration tests', 'The database schema from the ORM'], correct: 0, explanation: 'A queue buffers work so a slow or temporarily-down consumer doesn\'t block the producer.' },
    ],
  },
  {
    id: 'docker', num: 21, phase: 5, title: 'Docker & Containers',
    ...
    quiz: [
      { q: 'What problem does Docker primarily solve?', options: ['"Works on my machine" — packaging an app with its exact runtime environment', 'Automatically writing your application code', 'Replacing version control', 'Guaranteeing zero downtime deployments'], correct: 0, explanation: 'A container bundles the app, its dependencies, and OS-level config into one portable, reproducible unit.' },
      { q: 'What is the difference between a Docker image and a container?', options: ['An image is a read-only template; a container is a running instance of it', 'They are the same thing with different names', 'An image can only run once', 'A container is stored in the Docker Hub, an image is not'], correct: 0, explanation: 'You can start many containers from the same image, each an isolated running instance.' },
      { q: 'What does docker-compose let you do that a single Dockerfile doesn\'t?', options: ['Define and run multiple linked containers (app, DB, cache) together', 'Build smaller images', 'Skip writing a Dockerfile entirely', 'Automatically scale to multiple servers'], correct: 0, explanation: 'Compose orchestrates multiple services with shared networking/volumes from one YAML file — useful for local dev stacks.' },
      { q: 'Why use a multi-stage Dockerfile build?', options: ['To keep the final image small by excluding build-only tools/dependencies', 'To run tests automatically', 'To support multiple programming languages in one container', 'To enable horizontal scaling'], correct: 0, explanation: 'A multi-stage build compiles in one stage and copies only the needed artifacts into a lean final image.' },
      { q: 'What does a Docker volume provide that the container\'s own filesystem doesn\'t?', options: ['Data that persists beyond the container\'s lifecycle', 'Faster CPU performance', 'Network isolation', 'Automatic image versioning'], correct: 0, explanation: 'A container\'s filesystem is ephemeral by default — a volume is how you persist data like a database\'s files.' },
    ],
  },
  {
    id: 'kubernetes', num: 22, phase: 5, title: 'Kubernetes & Orchestration',
    ...
    learnMore: { label: 'Kubernetes — Concepts', url: 'https://kubernetes.io/docs/concepts/' },
    subtopics: [
      { title: 'Core Objects',
        concepts: [
        ...
        q: [
          { t: 'Deploy a Spring Boot app to a local Kubernetes cluster (minikube/kind) with a Deployment and Service', d: 'M', p: 'Build task' },
          { t: 'Move a hardcoded database URL out of the image and into a ConfigMap and Secret', d: 'M', p: 'Build task' },
          { t: 'Manually scale a Deployment to 3 replicas and verify traffic is distributed across all of them', d: 'E', p: 'Build task' },
        ] },
      { title: 'Scaling & Operations',
        ...
        learnMore: { label: 'Kubernetes — Pod Lifecycle', url: 'https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle/' },
        checklist: [
        ...
        q: [
          { t: 'Set up a Horizontal Pod Autoscaler that scales a Deployment based on CPU usage', d: 'M', p: 'Build task' },
          { t: 'Trigger a rolling update to a new image version, then roll it back after simulating a bad deploy', d: 'M', p: 'Build task' },
          { t: 'Add readiness and liveness probes to a Spring Boot Deployment using its Actuator health endpoint', d: 'M', p: 'Build task' },
        ] },
    ],
    mini: { title: 'Checkpoint Project — Kubernetes Redeploy', desc: 'Take the docker-compose stack from the Docker topic and re-deploy it to Kubernetes with a Deployment, Service, ConfigMap, and a horizontal pod autoscaler.' },
    quiz: [
      { q: 'What is a Kubernetes Pod?', options: ['The smallest deployable unit — one or more containers sharing network/storage', 'A single physical server', 'A backup of a container image', 'A Kubernetes configuration file'], correct: 0, explanation: 'A Pod wraps one or more tightly-coupled containers that should always run together on the same node.' },
      { q: 'What does a Kubernetes Service provide?', options: ['A stable network endpoint that routes to a changing set of Pods', 'A backup mechanism for Pods', 'A way to build container images', 'A CI/CD pipeline'], correct: 0, explanation: 'Pods come and go (rescheduled, scaled, restarted); a Service gives clients one stable address regardless.' },
      { q: 'What does a Horizontal Pod Autoscaler do?', options: ['Automatically adds/removes Pod replicas based on metrics like CPU usage', 'Increases a single Pod\'s CPU/memory limits', 'Balances load across multiple clusters', 'Encrypts traffic between Pods'], correct: 0, explanation: 'HPA scales the NUMBER of pod replicas out or in — "horizontal" scaling — in response to observed load.' },
      { q: 'What is the purpose of a ConfigMap in Kubernetes?', options: ['Externalize configuration data from container images', 'Store encrypted secrets', 'Define network policies', 'Schedule Pods onto specific nodes'], correct: 0, explanation: 'ConfigMaps let you change config without rebuilding the image — Secrets are the equivalent for sensitive values.' },
      { q: 'What does a Kubernetes Deployment manage?', options: ['Desired Pod replica count and rolling updates for a set of Pods', 'Physical server provisioning', 'DNS records for external domains', 'Docker image building'], correct: 0, explanation: 'A Deployment declares the desired state (replica count, image version) and Kubernetes reconciles the actual state to match, including rolling updates.' },
    ],
  },
  {
    id: 'cicd', num: 23, phase: 5, title: 'CI/CD & Deployment',
    ...
    quiz: [
      { q: 'What does Continuous Integration (CI) primarily ensure?', options: ['Every code change is automatically built and tested before merging', 'Code is deployed to production automatically on every commit', 'All developers work on the same branch', 'Tests are written after the feature ships'], correct: 0, explanation: 'CI catches integration problems early by running the build/test suite on every push, not deployment itself.' },
      { q: 'What is the difference between Continuous Delivery and Continuous Deployment?', options: ['Delivery keeps every change release-ready but requires a manual trigger; Deployment ships automatically', 'They are identical concepts', 'Delivery only applies to mobile apps', 'Deployment never runs tests first'], correct: 0, explanation: 'Continuous Deployment removes the manual approval gate that Continuous Delivery still keeps.' },
      { q: 'What does a health check endpoint (like Spring Boot Actuator\'s /health) enable?', options: ['Automated systems can detect if an instance is unhealthy and stop routing traffic to it', 'It replaces the need for logging', 'It automatically fixes bugs', 'It encrypts application traffic'], correct: 0, explanation: 'Load balancers and orchestrators poll health endpoints to decide whether an instance should receive traffic.' },
      { q: 'Why version-tag Docker images (e.g. app:1.4.2) instead of always using "latest"?', options: ['Reproducible deployments and safe rollbacks to a known-good version', 'Tagged images run faster', 'Docker requires a version tag to build at all', 'It reduces image size'], correct: 0, explanation: 'With "latest" you can\'t reliably roll back — you don\'t know what was actually running.' },
      { q: 'What is the purpose of a rollback strategy in a deployment pipeline?', options: ['Quickly redeploy a known-good previous version if the new one breaks in production', 'Automatically fix bugs in the new version', 'Prevent all deployments from failing', 'Replace manual testing entirely'], correct: 0, explanation: 'A fast, tested rollback path turns a bad deploy into a brief incident instead of an extended outage.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'sysdesign', num: 24, phase: 6, title: 'System Design Basics',
    ...
    quiz: [
      { q: 'What does the CAP theorem state?', options: ['A distributed system can guarantee at most two of Consistency, Availability, and Partition tolerance at once', 'A system must always choose Consistency over Availability', 'CAP only applies to relational databases', 'CAP theorem is about caching strategies'], correct: 0, explanation: 'Since network partitions are unavoidable in practice, the real-world choice is usually between consistency and availability during a partition.' },
      { q: 'What is a circuit breaker pattern used for?', options: ['Stopping repeated calls to a failing downstream service to prevent cascading failure', 'Encrypting network traffic', 'Load balancing across servers', 'Compressing HTTP responses'], correct: 0, explanation: 'Once failures cross a threshold, the circuit "opens" and fails fast instead of piling up slow, doomed requests.' },
      { q: 'Why does a URL shortener design typically favor a base62-encoded counter or hash over sequential integers?', options: ['To produce short, non-guessable, and evenly distributed keys', 'Sequential IDs are impossible to store in a database', 'Base62 is required by HTTP', 'It makes the database schema simpler'], correct: 0, explanation: 'A short alphanumeric code keeps URLs compact while avoiding easily-guessable, predictable sequences.' },
      { q: 'What does a load balancer\'s health check accomplish?', options: ['Stops routing traffic to instances that are unhealthy or unresponsive', 'Increases the speed of every request', 'Automatically scales the database', 'Encrypts traffic between services'], correct: 0, explanation: 'Removing unhealthy instances from rotation prevents user requests from hitting a broken backend.' },
      { q: 'Why is a CDN effective for scaling a read-heavy, mostly-static workload?', options: ['It caches content at edge locations physically closer to users, reducing origin load and latency', 'It replaces the need for a database entirely', 'It only works for video content', 'It requires no configuration'], correct: 0, explanation: 'Serving from a nearby edge node is both faster for the user and takes load off your origin servers.' },
    ],
  },
  {
    id: 'cloud', num: 25, phase: 6, title: 'Cloud Fundamentals',
    ...
    quiz: [
      { q: 'What is a key benefit of a managed database service (e.g. RDS, Cloud SQL) over self-hosting Postgres on a VM?', options: ['The provider handles backups, patching, and failover for you', 'It is always free', 'It removes the need for indexes', 'It guarantees zero latency'], correct: 0, explanation: 'Managed services trade some control for offloading operational burden like backups, patching, and HA setup.' },
      { q: 'What does a VPC (Virtual Private Cloud) provide?', options: ['An isolated, private network boundary for your cloud resources', 'A content delivery network', 'A managed database', 'A CI/CD pipeline'], correct: 0, explanation: 'A VPC lets you control exactly which resources can reach each other and the public internet.' },
      { q: 'What is the purpose of a security group in cloud networking?', options: ['A virtual firewall controlling inbound/outbound traffic for specific resources', 'A group of users with admin access', 'A backup schedule configuration', 'A DNS routing rule'], correct: 0, explanation: 'Security groups define exactly which ports/sources are allowed to reach a given instance or service.' },
      { q: 'Why use object/blob storage (e.g. S3) instead of storing files on a server\'s local disk?', options: ['Durability and scalability independent of any single server\'s lifecycle', 'It\'s always faster than local disk', 'It requires no access control', 'Local disk storage is deprecated'], correct: 0, explanation: 'A server can be replaced or scaled without losing files, since the storage lives outside the compute instance itself.' },
      { q: 'What is the principle of least privilege in cloud IAM?', options: ['Grant only the permissions a resource/user actually needs, nothing more', 'Grant admin access to every service by default', 'Disable all permissions until a support ticket is filed', 'Only apply to production environments'], correct: 0, explanation: 'Minimizing granted permissions limits the damage if credentials are ever compromised.' },
    ],
  },
  ...
  // ==================================================================
  {
    id: 'html-css', num: 26, phase: 7, title: 'HTML, CSS & Responsive Design',
    ...
    quiz: [
      { q: 'Why prefer semantic HTML elements (<nav>, <article>, <button>) over generic <div>s?', options: ['They convey meaning to screen readers and improve accessibility/SEO for free', 'They render faster in every browser', 'They require less CSS', 'Semantic elements cannot be styled'], correct: 0, explanation: 'A screen reader can announce "navigation" or "button" from the tag itself — a <div> gives it nothing.' },
      { q: 'What is the main difference between Flexbox and CSS Grid?', options: ['Flexbox is one-dimensional (row or column); Grid is two-dimensional (rows and columns together)', 'Grid only works in Chrome', 'Flexbox cannot wrap items', 'They produce identical layouts always'], correct: 0, explanation: 'Reach for Flexbox for a single row/column of items, and Grid when you need to control both axes at once.' },
      { q: 'What does a mobile-first media query approach mean?', options: ['Base styles target small screens; larger breakpoints add complexity with min-width queries', 'Only mobile devices are supported', 'Desktop styles are written first, then overridden for mobile', 'Media queries are unnecessary with mobile-first design'], correct: 0, explanation: 'Starting from the simplest (mobile) layout and progressively enhancing for larger screens is usually a cleaner CSS cascade.' },
      { q: 'What accessibility attribute would you add to an icon-only button with no visible text?', options: ['aria-label describing the action', 'tabindex="-1" to remove it from focus', 'role="presentation"', 'alt text is sufficient on a <button>'], correct: 0, explanation: 'Without visible text, a screen reader has nothing to announce unless you provide an aria-label.' },
      { q: 'What CSS property is most directly responsible for an element\'s stacking order relative to siblings?', options: ['z-index (within a positioned stacking context)', 'font-weight', 'line-height', 'text-align'], correct: 0, explanation: 'z-index controls layer order, but only takes effect on positioned elements (relative/absolute/fixed/sticky) within a stacking context.' },
    ],
  },
  {
    id: 'js-dom', num: 27, phase: 7, title: 'JavaScript Core & DOM',
    ...
    quiz: [
      { q: 'What is the difference between == and === in JavaScript?', options: ['=== checks value and type without coercion; == coerces types before comparing', 'They are functionally identical', '=== is deprecated', '== is only for numbers'], correct: 0, explanation: '== can produce surprising results (e.g. "0" == false is true); === is almost always the safer default.' },
      { q: 'What does the JavaScript event loop enable?', options: ['Non-blocking async operations on a single thread via a callback/microtask queue', 'True multi-threaded parallel execution', 'Automatic garbage collection of closures', 'Synchronous blocking of the entire page during a fetch'], correct: 0, explanation: 'JS is single-threaded, but the event loop lets async work (timers, fetch) run without blocking the main thread.' },
      { q: 'What is a closure in JavaScript?', options: ['A function that retains access to variables from its enclosing scope after that scope has returned', 'A method that closes a database connection', 'A way to prevent a variable from being reassigned', 'A synonym for an IIFE'], correct: 0, explanation: 'Closures are why a function returned from another function can still "remember" variables from where it was created.' },
      { q: 'What is the difference between Promise.then() and async/await?', options: ['async/await is syntactic sugar over promises, making async code read like synchronous code', 'async/await is faster at runtime', 'Promises cannot be used with async/await', 'await blocks the entire browser tab'], correct: 0, explanation: 'Under the hood, both are built on the same Promise mechanism — await just makes chaining more readable.' },
      { q: 'Why can directly mutating the DOM in a tight loop hurt performance?', options: ['Each mutation can trigger a reflow/repaint, so batching changes is faster', 'The DOM only allows one mutation per second', 'JavaScript cannot mutate the DOM more than once', 'Mutating the DOM always throws an error in loops'], correct: 0, explanation: 'Frameworks like React batch DOM updates for exactly this reason — many small unbatched writes are expensive.' },
    ],
  },
  {
    id: 'typescript', num: 28, phase: 7, title: 'TypeScript Fundamentals',
    ...
    quiz: [
      { q: 'What is the main benefit TypeScript adds over plain JavaScript?', options: ['Compile-time type checking that catches errors before the code ever runs', 'Faster runtime execution', 'Automatic UI rendering', 'Built-in state management'], correct: 0, explanation: 'TypeScript compiles away entirely — its value is entirely in catching bugs and improving tooling before runtime.' },
      { q: 'What does an `interface` in TypeScript primarily describe?', options: ['The shape of an object — what properties/methods it must have', 'A runtime class you can instantiate', 'A database schema', 'A CSS style rule'], correct: 0, explanation: 'Interfaces are a compile-time-only contract; they don\'t exist in the compiled JavaScript output.' },
      { q: 'What is a generic (e.g. `function identity<T>(x: T): T`) used for?', options: ['Writing reusable, type-safe code that works across multiple types without using `any`', 'Making a function run faster', 'Disabling type checking for that function', 'Converting a function into a class'], correct: 0, explanation: 'Generics let a function/type stay flexible while still preserving specific type information at each call site.' },
      { q: 'Why is `any` generally discouraged in a TypeScript codebase?', options: ['It disables type checking for that value, defeating the purpose of using TypeScript', 'It makes the code run slower', 'It is not valid TypeScript syntax', '`any` only works with numbers'], correct: 0, explanation: 'Reaching for `any` to "make an error go away" silently reintroduces the exact bugs TypeScript is meant to catch.' },
      { q: 'What does a union type like `string | number` mean?', options: ['A value can be either a string or a number', 'A value must be both a string and a number simultaneously', 'It defines a new class', 'It is only valid inside generics'], correct: 0, explanation: 'Union types express "one of these possible types," and TypeScript narrows which one you have based on runtime checks.' },
    ],
  },
  {
    id: 'react-fundamentals', num: 29, phase: 7, title: 'React Fundamentals',
    ...
    quiz: [
      { q: 'What triggers a React component to re-render?', options: ['Its state or props changing', 'The browser window resizing', 'A page reload only', 'Every setInterval tick regardless of state'], correct: 0, explanation: 'React schedules a re-render whenever a component\'s own state changes (via its setter) or its props change.' },
      { q: 'What does the useEffect cleanup function (the function it returns) prevent?', options: ['Leaks from subscriptions/timers continuing after a component unmounts or deps change', 'The component from rendering at all', 'State from ever updating', 'Props from being passed down'], correct: 0, explanation: 'Without cleanup, an interval or subscription set up in useEffect keeps running even after the component is gone.' },
      { q: 'What does "lifting state up" mean?', options: ['Moving shared state to the nearest common ancestor of the components that need it', 'Storing state in a global variable', 'Converting state to props permanently', 'Moving state into a CSS file'], correct: 0, explanation: 'It\'s the default way to share data between sibling components before reaching for Context or a state library.' },
      { q: 'What is a controlled input in React?', options: ['An input whose value is driven entirely by React state, updated on every change', 'Any input element regardless of implementation', 'An input that cannot be edited by the user', 'An input validated only on the server'], correct: 0, explanation: 'React state is the single source of truth — every keystroke updates state, which renders back into the input\'s value.' },
      { q: 'What is a custom hook?', options: ['A function starting with "use" that calls other hooks to extract reusable stateful logic', 'A hook that only works in class components', 'A CSS utility for hover effects', 'A built-in React API for routing'], correct: 0, explanation: 'Custom hooks are the standard way to share stateful logic (like a fetch-with-loading pattern) across multiple components.' },
    ],
  },
  {
    id: 'react-advanced', num: 30, phase: 7, title: 'React Advanced & State Management',
    ...
    quiz: [
      { q: 'What problem does React Context solve?', options: ['Passing a value to deeply nested components without prop-drilling through every layer', 'Making a component render faster', 'Replacing the need for props entirely', 'Handling HTTP requests directly'], correct: 0, explanation: 'A value in Context can be read by any descendant, skipping intermediate components that don\'t use it themselves.' },
      { q: 'Why might useMemo or useCallback fail to actually improve performance if used everywhere?', options: ['They have their own comparison/memoization cost, which can outweigh the benefit for cheap computations', 'They are deprecated in modern React', 'They only work with class components', 'They break component rendering entirely'], correct: 0, explanation: 'Memoization isn\'t free — apply it where profiling shows a real, expensive re-render/recompute problem.' },
      { q: 'What does list virtualization solve?', options: ['Rendering only the currently-visible items of a very long list, keeping scrolling smooth', 'Making API calls faster', 'Reducing bundle size', 'Improving SEO for a list page'], correct: 0, explanation: 'Rendering thousands of DOM nodes at once is slow — virtualization renders only what\'s in (or near) the viewport.' },
      { q: 'What is a benefit of a data-fetching library (React Query/SWR) over raw fetch + useEffect?', options: ['Built-in caching, revalidation, and request deduplication', 'It eliminates the need for an API entirely', 'It replaces React itself', 'It only works with GraphQL'], correct: 0, explanation: 'These libraries solve problems (stale data, duplicate in-flight requests, refetch-on-focus) that most apps eventually hit hand-rolling their own fetching logic.' },
      { q: 'Why must every list endpoint explicitly handle loading, error, AND empty states?', options: ['"It just works when there\'s data" is an incomplete implementation — each state needs deliberate UI', 'React automatically handles all three', 'Only the error state actually matters in practice', 'Empty and loading states are visually identical by default'], correct: 0, explanation: 'A missing empty-state or error-state is one of the most common real-world UI bugs in production apps.' },
    ],
  },
];
