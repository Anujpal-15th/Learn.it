...
//
// Shapes:
//   PHASES:         [{ name, desc, tier, learnMore? }]
//     tier drives the 5-section UI grouping (roadmap page), without changing
//     phase numbering/prerequisites: 'core-foundations' | 'dsa' | 'backend' |
//     'production-advanced' | 'system-design' | 'optional'.
...
// sysdesign) — existing progress on those specific old subtopics is orphaned,
// not deleted, and stops counting toward anything. See
...
export const TOPICS = [
  ...
  },
  {
    ...
    "mini": {
      "title": "Checkpoint Project — Schema + Query Tuning Lab",
      "desc": "Design a normalized PostgreSQL schema for an e-commerce store, wire it up through a pooled connection with Flyway migrations, then deliberately break and fix slow queries with indexes."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "mini": {
      "title": "Checkpoint Project — Multi-Module Utility Library",
      "desc": "A shared internal library split into modules (validation, logging, common-dto), built and versioned with Gradle."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "mini": {
      "title": "Checkpoint Project — Inventory Persistence Layer",
      "desc": "A JPA layer for a warehouse system using Spring Data JPA repositories: entities, relationships, and one deliberately-fixed N+1 problem with before/after query logs."
    },
    "quiz": [
      {
      ...
      },
      {
        ...
        "options": [
          "LAZY loads the association only when accessed; EAGER loads it immediately with the parent",
          "LAZY is faster in every case",
          "EAGER only works with @OneToMany",
          "They behave identically in practice"
        ],
        "correct": 0,
        "explanation": "LAZY defers the extra query until the association is actually touched, which is usually safer against accidental N+1s."
      },
      {
        "q": "What does @Transactional on a Spring service method do?",
        "options": [
          "Wraps the method in a database transaction, committing or rolling back as a unit",
          "Marks the method as thread-safe",
          "Caches the method's return value",
          "Makes the method asynchronous"
        ],
        "correct": 0,
        "explanation": "It's Spring's declarative way of applying transaction boundaries via an AOP proxy around the method."
      },
      {
        "q": "How would you fix an N+1 problem for a @OneToMany association in JPA?",
        "options": [
          "Use a JOIN FETCH (or an entity graph) to load it in one query",
          "Switch the database to NoSQL",
          "Disable the ORM cache",
          "Add more indexes"
        ],
        "correct": 0,
        "explanation": "JOIN FETCH tells JPA to eagerly fetch the association in the same query, avoiding the per-row follow-up queries."
      },
      {
        "q": "Why does calling an @Transactional method from within the same class often fail to start a transaction?",
        "options": [
          ...
          "Transactional annotations only work on interfaces",
          "The method must be static",
          "JPA disables transactions inside loops"
        ],
        "correct": 0,
        "explanation": "The proxy that applies @Transactional wraps external calls to the bean — a direct `this.method()` call skips the proxy entirely."
      }
  ...
  },
  {
    ...
    "mini": {
      "title": "Checkpoint Project — Auth Service",
      "desc": "A standalone authentication microservice: signup, login, JWT issuing, role-based route protection, hardened against SQL injection, XSS, and CSRF."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "mini": {
      "title": "Combined Project — Fully Tested Auth Service",
      "desc": "Add a complete unit + integration test suite (with Testcontainers, @WebMvcTest and @DataJpaTest slices) to the Auth Service project."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "mini": {
      ...
      "desc": "Orders published as events, consumed asynchronously (idempotently) to update inventory and send notifications, with Redis caching on reads."
    },
    "quiz": [
      {
      ...
      {
        "q": "Why must a message consumer be idempotent in an event-driven system?",
        "options": [
          "Message brokers can deliver the same message more than once",
          "Idempotency makes messages process faster",
          "It's required by the JMS specification",
          "Idempotency prevents any message loss"
        ],
        "correct": 0,
        "explanation": "At-least-once delivery is common — a consumer that isn't idempotent could double-charge or double-ship on a redelivered message."
      },
      {
      ...
      {
        "q": "What does a message queue decouple in a system?",
        "options": [
          "Producers from consumers, letting them operate at different speeds independently",
          "The frontend from the CSS framework",
          "Unit tests from integration tests",
          "The database schema from the ORM"
        ],
        "correct": 0,
        "explanation": "A queue buffers work so a slow or temporarily-down consumer doesn't block the producer."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "mini": {
      ...
      "desc": "Package the Order Events Pipeline (API + DB + Redis + MQ) into a single docker-compose stack."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "mini": {
      ...
      "desc": "Full CI/CD: push to main → tests run → image built → deployed to your Azure VM automatically, with Actuator + Prometheus/Grafana observability."
    },
    "quiz": [
      {
  ...
  },
  {
    ...
    "quiz": [
      ...
      },
      {
        ...
        "options": [
          "Splitting one logical database across multiple physical machines by a key",
          "Creating a backup copy of the database",
          "Adding an index to speed up queries",
          "Encrypting data at rest"
        ],
        "correct": 0,
        "explanation": "Sharding is how you scale writes/storage beyond what a single machine can hold, at the cost of cross-shard query complexity."
      },
      {
      ...
      {
        "q": "What does database replication provide?",
        "options": [
          "Read scaling and failover via copies of data on other machines",
          "Automatic schema migration",
          "Query result caching",
          "Data compression"
        ],
        "correct": 0,
        "explanation": "A replica can serve read traffic and take over if the primary fails, though it doesn't by itself solve write scaling."
      }
    ]
  },
  {
